...
if (!in_array($modx->event->name, $allowedEvents)) {
    return;
}
...
if ($modx->event->name === 'OnChunkFormSave' && !in_array($chunk->get('name'), $chunks)) {
    return;
}
...
if (empty($csssCustomCssPath)) {
    $csssCustomCssPath = $modx->getOption('assets_path') . 'components/csssweet/' . $mode . '/';
}
...
if (file_exists($file) && is_readable($file)) {
    $modx->log(modX::LOG_LEVEL_INFO, 'Success! Custom CSS saved to file "' . $file . '"', '', 'saveCustomCss');
}
