...
$cssSweetPath .= 'model/';
if (file_exists($cssSweetPath . 'csssweet.class.php')) $csssweet = $modx->getService('csssweet', 'CssSweet', $cssSweetPath, $scriptProperties);
...
if (!($csssweet instanceof CssSweet)) {
    ...
    // Init scssphp
    if ($csssweet->scssphpInit()) {
    
        $scssMin = new Leafo\ScssPhp\Compiler();
        // If we got the class, try minification and scss processing. Log failures.    
        if ($scssMin instanceof Leafo\ScssPhp\Compiler) {
